...
from api_contracts import dashboard_document, trajectory_resource, validate_document
from supabase_repository import SupabasePucRepository
from logging_utils import logger, timed
from trajectory_repository import Phase7TrajectoryRepository, run_identity_pipeline
from traffic_analytics import Phase8Analytics, ProductionCameraNetwork
...
class Phase9Service:
    ...
    def refresh(self) -> Phase9Snapshot:
        ...
        logger.info("snapshot_ready trajectories=%s alerts=%s flows=%s", len(records), len(alerts), len(flows))
...
def build_local_demo_service() -> tuple[Phase9Service, tempfile.TemporaryDirectory[str]]:
    ...
    repository = Phase7TrajectoryRepository(Path(directory.name) / "local_demo_store.json")
